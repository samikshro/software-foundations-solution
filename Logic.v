...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P HP Q HP'. unfold not in HP. apply HP in HP'. destruct HP'.
Qed.
(** [] *)
...
Proof.
  ...
  apply H in HP. apply HNQ in HP. destruct HP.
Qed.
(** [] *)
...
Proof.
  intros X P H. unfold not. intros [x H']. apply H' in H. destruct H.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. split.
    + intros [].
    + intros [x [Hl Hr]]. destruct Hr.
  - simpl. split.
    + intros [Hf | Hin].
      * exists x. split.
        -- apply Hf.
        -- left. reflexivity.
      * apply IHl in Hin. destruct Hin. destruct H. exists x0. split.
        -- apply H.
        -- right. apply H0.
    + intros [x' [Hl [Hr1 | Hr2]]].
      * left. rewrite <- Hr1 in Hl. apply Hl.
...
Proof.
  ...
  - intros H. induction l.
    ...
    + intros x0 H'. simpl in H. destruct H as [H1 H2]. simpl in H'. destruct H'.
      * rewrite <- H. apply H1.
      * apply IHl.
        -- apply H2.
        -- apply H.
...
Proof.
  intros Podd Peven n Ho He. unfold combine_odd_even. destruct (oddb n).
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros Podd Peven n Hc Ho. unfold combine_odd_even in Hc. destruct (oddb n) eqn:E in Hc.
  - apply Hc.
  - rewrite E in Ho. discriminate Ho.
Qed.
...
Proof.
  ...
  intros l. unfold tr_rev. rewrite rev_append_app. apply app_nil_r with (l:=rev l).
Qed.
